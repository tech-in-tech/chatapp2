...
import { geySocket } from '../lib/socket.js'
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-10 h-10 rounded-full overflow-hidden border shrink-0 ${isSender ? "order-2 ml-3" : "order-1 mr-3"}`}>
                    <img className='w-full h-full object-cover' src={isSender ? authUser?.avatar?.url || "/vite.svg" : selectedUser?.avatar?.url || "/vite.svg"} alt="" />
                  </div>
 
                  {/* Bubble */}
                  <div className={`max-w-xs sm:max-w-md px-4 py-2 rounded-xl text-sm ${isSender ? "bg-blue-400/20 text-black order-1" : "bg-gray-200 text-black order-2"}`}>
